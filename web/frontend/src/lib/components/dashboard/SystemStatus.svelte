  ...
  <div slot="header" class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div class="flex items-center gap-2">
        <div
          class="w-3 h-3 rounded-full {getConnectionColor(
    ...
    <div class="flex items-center gap-2">
      ...
      >
        <Icon
          ...
          class="w-4 h-4 {!autoRefresh ? 'opacity-50' : ''}"
        />
        Auto-refresh
      </Button>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center p-3 bg-gray-50 rounded-lg">
          ...
          <span class="text-xs text-gray-600"
            >{$connectionStatus.api ? "Connected" : "Disconnected"}</span
          >
        </div>
        ...
        <div class="text-center p-3 bg-gray-50 rounded-lg">
          ...
          <span class="text-xs text-gray-600"
            >{$connectionStatus.qdrant ? "Connected" : "Disconnected"}</span
          >
        </div>
        ...
        <div class="text-center p-3 bg-gray-50 rounded-lg">
          ...
          <span class="text-xs text-gray-600"
            >{$connectionStatus.ollama ? "Connected" : "Disconnected"}</span
          >
        </div>
        ...
        <div class="text-center p-3 bg-gray-50 rounded-lg">
          ...
          <span class="text-xs text-gray-600"
            >{$connectionStatus.websocket ? "Connected" : "Disconnected"}</span
          >
        </div>
      </div>
            ...
            {#each health.services as service}
              ...
              >
                <div class="flex items-center gap-3">
                  <div class="flex items-center gap-2">
                    <div
                      class="w-2 h-2 rounded-full {service.status === 'healthy'
                        ? 'bg-green-500'
                        : 'bg-red-500'}"
                    ></div>
                ...
                <div class="text-right text-sm text-gray-600">
                  ...
                  {/if}
                  {#if service.error}
                    <div class="text-xs text-red-600 max-w-xs truncate">
                      {service.error}
                    </div>
                  {/if}
                </div>
