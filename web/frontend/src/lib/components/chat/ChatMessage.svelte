...
>
  ...
  <!-- Message Content -->
  <div
    class="flex-1 min-w-0 {message.role === 'user'
      ? 'text-right'
      : 'text-left'}"
  >
    <!-- Message Bubble -->
    ...
    <!-- Message Metadata -->
    <div
      class="flex items-center gap-2 mt-1 text-xs text-gray-500 {message.role ===
      'user'
        ? 'justify-end'
        : 'justify-start'}"
    >
      ...
      {#if message.sources && message.sources.length > 0}
        <span>•</span>
        <button class="hover:text-blue-500 underline" on:click={toggleSources}>
          {message.sources.length} source{message.sources.length !== 1
            ? "s"
            : ""}
        </button>
      {/if}
