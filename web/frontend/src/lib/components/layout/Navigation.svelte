<script lang="ts">
  ...
  const getNavItemClasses = (itemId: string, disabled: boolean = false) => {
    const baseClasses = 'flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200';
...
<nav class="h-full flex flex-col">
  ...
  <div class="flex items-center justify-between p-4 border-b border-gray-200">
    ...
    >
      <Icon icon={collapsed ? "material-symbols:chevron-right" : "material-symbols:chevron-left"} class="w-4 h-4" />
    </Button>
  </div>
  ...
  <div class="p-4 border-t border-gray-200">
    ...
    {:else}
      <div class="flex justify-center">
        <div class="w-2 h-2 bg-green-500 rounded-full" title="System Online"></div>
      </div>
    {/if}
  </div>
</nav>