...
async def get_collection_statistics(collection_name: str):
    ...
    try:
        ...
        return CollectionStats(
            ...
            dimension=stats.get("config", {}).get("params", {}).get("vectors", {}).get("size", 0),
...
async def find_similar_documents(document_id: int, collection: str = "articles", limit: int = 10):
    """Find documents similar to the given document."""
    try:
        search_service = get_search_service()
        search_results = await search_service.find_similar(document_id, collection, limit)
