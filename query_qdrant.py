...
def search_qdrant_hybrid(
    ...
    try:
        ...
        fusion = models.Fusion.RRF if fusion_method.lower() == "rrf" else models.Fusion.DBSF
...
def group_results_by_article(results: List[Dict[str, Any]]) -> Dict[int, List[Dict[str, Any]]]:
    """
...
def decode_chunk_id(chunk_id: int) -> tuple[int, int]:
    """
    Decode integer chunk ID back to article_id and chunk_index.
    
    Args:
        chunk_id: Integer ID in format (article_id * 100 + chunk_index)
    
    Returns:
        Tuple of (article_id, chunk_index)
    """
    article_id = chunk_id // 100
...
def get_article_by_id(client: QdrantClient, collection_name: str, article_id: int) -> List[Dict[str, Any]]:
    """
...
def format_grouped_results(grouped_results: Dict[int, List[Dict[str, Any]]], query: str) -> str:
...
def format_json_results(results: List[Dict[str, Any]], query: str, grouped: bool = False) -> str:
    ...
    while True:
        try:
            ...
            if query.lower() == 'help':
                ...
                print("  read <id>      - Read full article by ID (e.g., 'read 146412')")
...
Examples:
  ...
  python query_qdrant.py --article-id 146412 --query "makmum bersuara" --hybrid
    ...
    parser.add_argument("--collection", default="articles", help="Qdrant collection name")
    parser.add_argument("--ollama-url", default="http://localhost:11434", help="Ollama server URL")
    parser.add_argument("--model", default="embeddinggemma:latest", help="Ollama embedding model")
    
    # Search parameters
    parser.add_argument("--limit", type=int, default=10, help="Maximum number of results")
    parser.add_argument("--min-score", type=float, default=0.0, help="Minimum similarity score threshold")
    parser.add_argument("--article-id", type=int, help="Search within specific article only")
    parser.add_argument("--hybrid", action="store_true", help="Use hybrid search (vector + keyword matching)")
    parser.add_argument("--fusion-method", choices=["rrf", "dbsf"], default="rrf", 
                       help="Fusion method for hybrid search: RRF (Reciprocal Rank Fusion) or DBSF (Distribution-Based Score Fusion)")
    
    # Output options
    parser.add_argument("--output-format", choices=["detailed", "compact", "json"], default="detailed", 
                       help="Output format")
    parser.add_argument("--group-by-article", action="store_true", 
