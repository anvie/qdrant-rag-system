...
def chat_with_rag(
    ...
    query_vector = embed_one_ollama(
        query, args.embedding_model, args.ollama_url, session=session
    )
    ...
    augmented_prompt, sources = build_rag_prompt(
        query, context_chunks, max_context_length=args.max_context_length
    )
...
Examples:
  ...
  python chat_rag.py --system-prompt "You are a helpful Islamic scholar assistant."
    ...
    # Model arguments
    parser.add_argument(
        "--llm-model", default="llama2", help="Ollama LLM model for generation"
    )
    parser.add_argument(
        "--embedding-model",
    ...
    # RAG parameters
    parser.add_argument(
        "--top-k", type=int, default=5, help="Number of context chunks to retrieve"
    )
    parser.add_argument(
        "--min-score",
        type=float,
    ...
    parser.add_argument(
        "--max-tokens", type=int, default=2000, help="Maximum tokens in LLM response"
    )
    parser.add_argument("--system-prompt", type=str, help="System prompt for the LLM")

    # Display options
    parser.add_argument("--stream", action="store_true", help="Stream LLM responses")
    parser.add_argument(
        "--show-sources",
        action="store_true",
