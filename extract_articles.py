...
def parse_sql_values_line_by_line(sql_file: str, table_name: str) -> List[List]:
    ...
    with open(sql_file, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if line.startswith("CREATE TABLE") or (line.startswith("INSERT INTO") and table_name not in line):
                in_target_insert = False
...
def extract_articles_from_sql(sql_file: str) -> List[Dict]:
    ...
    articles.sort(key=lambda x: x['published_time'] if x['published_time'] else '', reverse=True)
...
Examples:
  ...
  python extract_articles.py --verbose
    ...
    # File arguments
    parser.add_argument("--input-file", default="articles.sql", 
                       help="SQL dump file to read from (default: articles.sql)")
    parser.add_argument("--output-file", default="articles.json", 
                       help="JSON file to write to (default: articles.json)")
    
    # Processing options
    parser.add_argument("--limit", type=int, 
                       help="Limit number of articles to extract")
    parser.add_argument("--published-only", action="store_true", 
                       help="Extract only published articles")
    parser.add_argument("--with-content-only", action="store_true", 
    ...
    parser.add_argument("--show-sample", action="store_true", default=True,
    ...
    try:
        ...
        with open(args.output_file, 'w', encoding='utf-8') as f:
            if args.indent:
                json.dump(articles, f, ensure_ascii=False, indent=args.indent, default=str)
            else:
        ...
        if not args.quiet:
            ...
            print(f"  With content: {articles_with_content} ({articles_with_content*100//len(articles) if articles else 0}%)")
            print(f"  With category: {articles_with_category} ({articles_with_category*100//len(articles) if articles else 0}%)")
            print(f"  Published: {published_articles} ({published_articles*100//len(articles) if articles else 0}%)")
            
            # Calculate content size statistics
            if articles_with_content > 0:
                content_lengths = [len(a['content']) for a in articles if a.get('content')]
            ...
            if args.show_sample and articles and articles_with_content > 0:
                ...
                if sample.get('caption'):
                    preview = sample['caption'][:100] + '...' if len(sample['caption']) > 100 else sample['caption']
                    print(f"  Caption: {preview}")
                
                if sample.get('content'):
                    preview = sample['content'][:150] + '...' if len(sample['content']) > 150 else sample['content']
